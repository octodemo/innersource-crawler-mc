...
if __name__ == "__main__":
    ...
    if "," in topic:
        # if the topic contains a comma, split it at each comma, and remove any whitespace
        topics = [t.strip() for t in topic.split(",")]
    else:
        # if the topic doesn't contain a comma, just make it a list
        topics = [topic]
    ...
    for topic in topics:
        ...
        for repo in all_repos:
            # Check if repo has already been added to repo_list
            if repo is not None and repo.repository.full_name not in repo_names:
                repo_names.add(repo.repository.full_name)
